...
<script>
    ...
    function changeWhen() {
    	//var radioValue = document.form1.when.value;  <% /* This is not working with IE. because the name when */ %>
    	var radioValue = $(":input:radio[name=when]:checked").val(); 
...
<div class="header-wrap Margin-bottom-5">
	<div class="header">
		<div class="header-title">
			<%=isEditMode ? Label.get("notify.edit.popup.title") : Label.get("notify.add.popup.title") %>
		</div>
...
<tr>
    ...
    <td>
        <% if(notify.getWhen() != null && notify.getWhen().equals("EO")) {%>
        	<label><input class='Radio Margin-right-5' type='radio' name='when' value='EO' checked onchange="changeWhen();"><%=Label.get("notify.end.ok")%></label>
        <%} else {%>
        	<label><input class='Radio Margin-right-5' type='radio' name='when' value='EO' onchange="changeWhen();"><%=Label.get("notify.end.ok")%></label>
        <%} %>
        <% if(notify.getWhen() == null || notify.getWhen().equals("EF")) {%>
        	<label><input class='Radio Margin-right-5' type='radio' name='when' value='EF' checked onchange="changeWhen();"><%=Label.get("notify.end.fail")%></label>
        <%} else {%>
        	<label><input class='Radio Margin-right-5' type='radio' name='when' value='EF' onchange="changeWhen();"><%=Label.get("notify.end.fail")%></label>
        <%} %>
        <% if(notify.getWhen() != null && notify.getWhen().equals("LONGRUN")) {%>
        	<label><input class='Radio Margin-right-5' type='radio' name='when' value='LONGRUN' checked onchange="changeWhen();"><%=Label.get("notify.longrun")%></label>
        <%} else {%>
        	<label><input class='Radio Margin-right-5' type='radio' name='when' value='LONGRUN' onchange="changeWhen();"><%=Label.get("notify.longrun")%></label>
        <%} %>
    </td>
</tr>
<tr id="LongRun">
	<th>
		<%=Label.get("notify.longrun")%>
	</th>
	<td>
