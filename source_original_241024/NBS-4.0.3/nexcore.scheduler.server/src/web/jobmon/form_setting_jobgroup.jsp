...
<script>
    ...
    function checkSubmit() {
    	if (document.form1.id.value.trim() == '') {
            ...
            alert('<%=Label.get("common.required.field.missing", Label.get("jobgroup.name"))%>');
            return false;
    	}
		else if (document.form1.parentId.value.trim() == '') {
            alert('<%=Label.get("common.required.field.missing", Label.get("jobgroup.parent.id"))%>');
            return false;
		}
...
<div class="header-wrap Margin-bottom-5">
	<div class="header">
		<div class="header-title">
			<%=isEditMode ? Label.get("jobgroup.edit.popup.title") : Label.get("jobgroup.add.popup.title") %>
		</div>
...
<tr>
    <th class="required"><%=Label.get("jobgroup.name")%></th>
...
<tr>
    <th class="required"><%=Label.get("jobgroup.parent.id")%></th>
