...
<%!
    ...
    String getClassNameOnly(nexcore.scheduler.entity.JobInstance jobins) {
        /* 
           JOB 종류가 JBATCH 또는 POJO 타입인 경우 패키지 명을 제외한 클래스 명을 반환한다.
           처리 중 에러가 발생하거나 JOB 종류가 JBATCH 또는 POJO 타입이 아닌 경우 null을 반환한다.
        */
        
        if ("JBATCH".equals(jobins.getJobType()) || "POJO".equals(jobins.getJobType())) {  /* 작업 종류가 EJB 또는 POJO 일 경우만 REPORT 파일 View */
        ...
        return null; /* 에러 또는 JOB 종류가 JBATCH 또는 POJO 타입이 아닌 경우 */
    ...
    /**
     * Job 타입에 따라 componentName 부분을 축약하여 표시한다.
     * JBATCH, POJO 일 경우는 클래스명만 리턴한다.
     */
    String getShortComponentNameNoTag(String jobType, String componentName) {
        if ("JBATCH".equals(jobType) || "POJO".equals(jobType)) { /* classname only */
        	...
        	if (componentName.indexOf("/") > -1) {
        		// unix 환경에서 / 를 기준으로 맨끝 파일명만 잘라냄
        		return componentName.substring(componentName.lastIndexOf("/")+1);
        	}else if (componentName.indexOf("\\") > -1) {
        		// windows 환경에서 \ 를 기준으로 맨끝 파일명만 잘라냄
        		return componentName.substring(componentName.lastIndexOf("\\")+1);
        	}else { // 파일명만 들어 있는 경우
    ...
    /**
     * Job 타입에 따라 componentName 부분을 축약하여 표시한다.
     * JBATCH, POJO 일 경우는 패키지명은 풍선도움말로 표시하고, 클래스명만 출력한다.
     */
